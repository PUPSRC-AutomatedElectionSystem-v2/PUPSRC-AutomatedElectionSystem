...
function applyFilters() {
    ...
    loadPage(
        "adminTable",
        "candidatePagination",
        searchTerm !== "" ? "submission_handlers/search-candidate.php" : "submission_handlers/fetch-candidate.php",
        1,
        searchTerm,
        "",
        "",
...
$(document).ready(function () {
    ...
    loadPage('adminTable', 'pagination', 'submission_handlers/fetch-candidate.php', 1);
    ...
    $('.sort-link').on('click', function (e) {
        ...
        loadPage('adminTable', 'pagination', 'submission_handlers/fetch-candidate.php', 1, $('#searchInput').val(), sortBy, sortOrder);
    });
});
...
$(document).ready(function () {
    ...
    $('.sort-link').on('click', function (e) {
        ...
        loadPage('adminTable', 'pagination', 'submission_handlers/fetch-candidate.php', 1, $('#searchInput').val(), sortBy, sortOrder, filterValues);
    });
    ...
    $("#searchInput").on("input", function () {
        ...
        if (searchTerm !== "") {
            loadPage(
                "adminTable",
                "candidatePagination",
                "submission_handlers/search-candidate.php",
                currentPage,
                searchTerm
            );
        } else {
            loadPage(
                "adminTable",
                "candidatePagination",
                "submission_handlers/fetch-candidate.php",
                currentPage,
                ""
            );
        }
...
function performDeletion(tableId, selectedIds) {
    $.ajax({
        url: "submission_handlers/move-trashbin-candidate.php",
        type: "POST",
...
function reloadPage(tableId) {
    const ajaxUrl = "submission_handlers/fetch-candidate.php";
    const paginationId = "candidatePagination";
...
$(document).ready(function () {
    ...
    $.ajax({
        url: 'submission_handlers/filter-candidate.php', // Replace with actual path
        method: 'GET',
        dataType: 'json',
