...
require_once FileUtils::normalizeFilePath('../model/configuration/registration-schedule-model.php');

class RegistrationSchedController extends RegistrationSchedModel
{
    ...
    private function validate()
    {
        // print_r($this->data);
        $isValid = [];
        if (!isset($this->data['registrationStart'])) {
            $this->client_error = 'ERR_MISSING_START_DATE';
            $isValid[] = false;
        }

        if (!isset($this->data['registrationEnd'])) {
        ...
        $startDateTime = DateTime::createFromFormat('Y-m-d\TH:i', $this->data['registrationStart']);
        $endDateTime = DateTime::createFromFormat('Y-m-d\TH:i', $this->data['registrationEnd']);
...
try {
    if ($_SERVER['REQUEST_METHOD'] === 'PUT') {
        ...
        $controller = new RegistrationSchedController();
