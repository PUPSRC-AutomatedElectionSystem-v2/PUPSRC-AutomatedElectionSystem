...
$(document).ready(function() {
    ...
    function handleDelete() {
        ...
        if (selectedIds.length > 0) {
            $.ajax({
                type: 'POST',
                url: 'submission_handlers/delete-selected-admin.php',
        ...
        rows.sort(function(a, b) {
            ...
            switch (sortOption) {
                case 'Most to Fewest Days':
                    keyA = parseInt($(a).children('td').eq(3).text().trim().split(' ')[0]);
                    keyB = parseInt($(b).children('td').eq(3).text().trim().split(' ')[0]);
                    console.log('Most to Fewest Days:', keyA, keyB);
                    return keyB - keyA;
                case 'Fewest to Most Days':
                    keyA = parseInt($(a).children('td').eq(3).text().trim().split(' ')[0]);
                    keyB = parseInt($(b).children('td').eq(3).text().trim().split(' ')[0]);
                    console.log('Fewest to Most Days:', keyA, keyB);
                    return keyA - keyB;
